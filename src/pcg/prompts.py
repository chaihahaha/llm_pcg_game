...
def chunk_task(seed: int, zone: Dict[str, Any], cx: int, cy: int, size: int) -> str:
    ...
    return (
        ...
        "★ npcs 的 x/y 必须落在【可通行】的地形上（草甸/高草/沙地/农田/道路/遗迹等），"
        "不要把人放在森林(T)、山岩(#)、水(~)、熔岩(!)里，否则玩家永远走不到他身边。"
    )
...
def evolve_task(lod_name: str, seed: int, scope_desc: str, local_digest: str,
    ...
    if period_hours and elapsed_hours > period_hours * 3 // 2:
        ...
        leap = (
            ...
            "这段时间跨度很大，events 至少给 2 条，分别落在不同阶段（早期/中期/后期），"
            "不要把所有事情都写成「现在」发生的。\n"
        )
    scope_line = ""
    if bbox:
    ...
    return (
        ...
        "★ 本层是更大世界的一部分：若【更高层级】或【本层往事】里有大事（战争、灾荒、迁徙、魔力潮汐），"
        "要让它们在本层留下可见的痕迹（逃难者经过、物价变化、巡逻加强、动物异常）。"
        "★ 标着 [已毁] 的物体已经不存在了，不要再次宣告它被毁，也不要让它在事件里正常运转。"
        "★ 事件必须对应你确实做出的 changes；没有做出改动就不要写相应的事件。"
        "★ 物品/样本在人物之间转移时，必须用 memory（如\"我把星髓样本交给了薇拉\"）留痕，"
        "否则后人无法解释它为何易主。"
        "★ NPC 的行为必须符合其身份(role)与阵营：头目不会亲自做技术分析、士兵不会替敌国辩解，"
        "除非档案里已经写明原因。"
        "★ memory 只能记录**亲历或确证**的事，不要让人物凭猜测断言国家层面的阴谋或高层勾结。"
        "★ 当一个物体被移动、摧毁或易主后，与它有关的 NPC 必须用新的 memory 更新认知"
        "（例如\"结晶已不在原先的埋藏处\"），不要留下与现状冲突的旧记忆。"
    )
