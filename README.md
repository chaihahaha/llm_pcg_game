...
| `auto <n>` | 自动探索 n 步 |
| `do <自由描述>` | **让大模型把你想要的任何事变成行动**；无法识别的输入也默认走这里 |
| `actions` | 查看已学会的动作（之后可直接 `do <名字>` 复用，不再花 LLM 调用） |
| `rules` | 查看被改写过的世界法则与钩子（运行时 monkey patch） |
...
模型会把文本翻译成一个**动作程序**：一串白名单效果（改地形 / 增删物体 / 移动 / 治疗 /
给予物品 / 记事件 / 改关系 / 写记忆…）+ 耗时；可复用的动作会被存成新指令。
它也可以选择**改写游戏本身**：

```
do 我要对这片土地施加诅咒：从此所有活着的东西说话不能超过十个字
```
模型会装上一条 `rule`（`npc_speech_max_chars=10`）或一段沙箱代码/hook，此后 NPC 真的只能
说十个字——而且写进存档，重开仍然生效。世界演化（`type=patch`）也能触发同类改动。
...
* 模型可以改游戏行为，但只能通过**白名单效果** + **AST 校验过的沙箱**：
  禁止 `import`/`while`/`with`/`class`/dunder 访问，整数常量有上限；
  坏补丁会被拒绝并在测存档时自动停用，不会把存档弄坏。
